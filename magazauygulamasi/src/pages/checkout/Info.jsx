...
export default function Info() {
    ...
    return (
        <>
            <Typography variant="subtitle2" color={{ color: "text.secondary" }}>Toplam</Typography>
            <Typography variant="h5" gutterBottom>{currenyTRY.format(total)}</Typography>
            <List>
                {cart?.cartItems.map((item) => (
                    <ListItem key={item.product.productId} sx={{ py: 1, px: 0 }}>
                        <ListItemAvatar>
                            <Avatar variant="square" src={`http://localhost:5001/images/${item.product.image}`}
                            ></Avatar>
                        </ListItemAvatar>
